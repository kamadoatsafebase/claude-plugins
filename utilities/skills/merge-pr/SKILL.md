...
- `prompt`: `/loop /utilities:merge-pr <PR_URL>` — this re-invokes the merge-pr skill via the `/loop` harness skill, which is what causes the skill to resume monitoring after the delay.
