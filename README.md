...
- **/plain-english `[text|file|description]`** — rewrites text to read human, concise, and simple: first removes AI writing patterns using its own vendored catalogue (`references/ai-patterns.md`, 25 patterns), then applies the generalizable subset of [ASD-STE100](https://www.asd-ste100.org/) Simplified Technical English (active voice, sentence and paragraph caps, three-word compound nouns, plain-word substitutions), skipping the aerospace-only rules. No external plugin needed.
...
- `/plain-english` vendors its AI-pattern catalogue from [blader/humanizer](https://github.com/blader/humanizer) v3.0.0, used under the MIT License (`utilities/skills/plain-english/references/LICENSE`). Re-sync it by hand when upstream changes.
